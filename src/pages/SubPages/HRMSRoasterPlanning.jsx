...
const RoasterModal1 = ({ isOpen, onClose, onSubmit }) => {
    const [employeeName, setEmployeeName] = useState('');
    const [role, setRole] = useState('');
    const [shiftStartTime, setShiftStartTime] = useState('');
    const [shiftEndTime, setShiftEndTime] = useState('');
    ...
    const columns = [
        {
            ...
            cell: (row) => (
                <div className="flex items-center gap-4">
                    <Link to={`/admin/hrms-roaster-planning-details/${row.id}`}>
                        <BsEye size={15} />
                    </Link>
                    <button onClick={handleAddClick1}>
                        <BiEdit size={15} />
                    </button>
                </div>
            ),
...
export default HRMSRoasterPlanning;
